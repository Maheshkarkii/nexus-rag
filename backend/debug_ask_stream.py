...
async def test():
    async with async_session_factory() as session:
        rag = get_rag_service()
        doc_id = uuid.UUID('2125b14b-75ac-40de-9aae-a3b9593b6ac2')
        proj_id = uuid.UUID('d8b7cdd1-ca20-483c-ae65-cb944d77270d')
        print("Calling ask_question_stream...")
        async for event in rag.ask_question_stream(
            session=session,
            project_id=proj_id,
